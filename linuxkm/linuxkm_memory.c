...
#undef CONFIG_X86_DEBUG_FPU /* avoid dependence on "alternatives_patched" and "xfd_validate_state()". */
#include "../kernel/fpu/internal.h"
#include "../kernel/fpu/xstate.h"
_Pragma("GCC diagnostic pop");

union wc_linuxkm_fpu_savebuf {
...
#ifdef WOLFSSL_COMMERCIAL_LICENSE
    wc_linuxkm_fpu_savebufs = (union wc_linuxkm_fpu_savebuf *)malloc(wc_linuxkm_fpu_states_n_tracked * sizeof(*wc_linuxkm_fpu_savebufs));
    if (! wc_linuxkm_fpu_savebufs) {
        pr_err("allocation of %lu bytes for "
               "wc_linuxkm_fpu_savebufs failed.\n",
               WC_LINUXKM_ROUND_UP_P_OF_2(wc_linuxkm_fpu_states_n_tracked) * sizeof(*wc_linuxkm_fpu_savebufs));
...
static struct fpstate *wc_linuxkm_fpstate_buf_from_fpu_state(struct wc_thread_fpu_count_ent *state) {
